...
import { History, Trash2, RotateCcw, Download, Upload, GitCompare, X } from "lucide-react";
...
import { cvss40Metrics, cvss3Metrics, cvss2Metrics } from "@/lib/cvss/metrics-data";
...
function ComparisonDialog({ open, onOpenChange, selectedEntries, getSeverityColor }: ComparisonDialogProps) {
  ...
  const getMetricDifferences = () => {
    ...
    const differences: Array<{ key: string; first: string; second: string }> = [];
    const allKeys = new Set([...Object.keys(firstMetrics), ...Object.keys(secondMetrics)]);
