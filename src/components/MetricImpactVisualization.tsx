...
export function MetricImpactVisualization({ impacts, version }: MetricImpactVisualizationProps) {
  const getCategoryIcon = (category: string) => {
    if (category.toLowerCase().includes("exploit")) return <Zap className="h-5 w-5" />;
    if (category.toLowerCase().includes("impact")) return <Activity className="h-5 w-5" />;
    return <Shield className="h-5 w-5" />;
  };

  const getCategoryColor = (category: string): string => {
    if (category.toLowerCase().includes("exploit")) return "text-yellow-600 dark:text-yellow-400";
    if (category.toLowerCase().includes("impact")) return "text-red-600 dark:text-red-400";
